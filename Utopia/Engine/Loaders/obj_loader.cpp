// Written by "thebennybox": https://www.youtube.com/user/thebennybox
...
Model OBJModel::ToSitronModel()
{
	...
	for (unsigned int i = 0; i < numIndices; i++)
	{
		...
		if (it == normalModelIndexMap.end())
		{
			...
			normalModel.texCoords.push_back(currentTexCoord);
			normalModel.normals.push_back(currentNormal);
			normalModel.tangents.push_back(currentNormal);
			normalModel.biTangents.push_back(currentNormal);
		}
		else
			normalModelIndex = it->second;
		...
		if (previousVertexLocation == (unsigned int)-1)
		{
			...
			result.texCoords.push_back(currentTexCoord);
			result.normals.push_back(currentNormal);
			result.tangents.push_back(currentNormal);
			result.biTangents.push_back(currentNormal);
		}
		else
			resultModelIndex = previousVertexLocation;
	...
	if (!hasNormals)
	{
		...
		{
			result.normals[i] = normalModel.normals[indexMap[i]];
			result.tangents[i] = normalModel.normals[indexMap[i]];
			result.biTangents[i] = normalModel.normals[indexMap[i]];
		}
	}
...
unsigned int OBJModel::FindLastVertexIndex(const std::vector<OBJIndex*>& indexLookup, const OBJIndex * currentIndex, const Model & result)
{
	unsigned int start = 0;
	unsigned int end = indexLookup.size();
	unsigned int current = (end - start) / 2 + start;
